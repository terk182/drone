...
static void sensorsTask(void *param)
{
    ...
    while (1) {
        ...
        /* MPU data-ready interrupt. Wait a short time for the INT semaphore,
         * then ALWAYS read the data: if the INT pin is not wired / not pulsing
         * (common on clone MPU modules), this acts as a polling fallback. */
        xSemaphoreTake(sensorsDataReady, M2T(2));
...
void processAccGyroMeasurements(const uint8_t *buffer)
{
    ...
    */
    ...
    // Guard: if accScale is 0/NaN (bad clone calibration), fall back to 1.0
    float scale = accScale;
    if (!(scale > 0.01f) || scale != scale) {
      scale = 1.0f;
    }
#ifdef CONFIG_TARGET_ESPLANE_V1
    //accScaled.x = (accelRaw.x) * SENSORS_G_PER_LSB_CFG / accScale;
    accScaled.x = -(accelRaw.x) * SENSORS_G_PER_LSB_CFG / scale;
#else
    accScaled.x = -(accelRaw.x) * SENSORS_G_PER_LSB_CFG / scale;
#endif

    accScaled.y = (accelRaw.y) * SENSORS_G_PER_LSB_CFG / scale;
    accScaled.z = (accelRaw.z) * SENSORS_G_PER_LSB_CFG / scale;
...
static void sensorsDeviceInit(void)
{
    ...
    bool mpuOk = mpu6050TestConnection();
    if (!mpuOk) {
        // A clone MPU can latch the I2C bus after an ESP32 reset (no power
        // cycle). Try to recover the bus and re-test before giving up.
        for (int attempt = 0; attempt < 3 && !mpuOk; attempt++) {
            DEBUG_PRINTW("MPU6050 connection failed, recovering I2C bus and retrying (%d)...\n", attempt + 1);
            i2cdrvTryToRestartBus(I2C0_DEV);
            vTaskDelay(M2T(50));
            mpuOk = mpu6050TestConnection();
        }
    }
    ...
    // Clear all MPU interrupts, then enable the data-ready interrupt.
    // sensorsTask waits on the GPIO INT (CONFIG_MPU_PIN_INT) before reading
    // new accel/gyro data, so DATA_RDY must be enabled or no data is ever read.
...
static void sensorsSetupSlaveRead(void)
{
    ...
    mpu6050SetWaitForExternalSensorEnabled(false);    // only enabled if external slaves exist (see below)
    ...
    // Enable I2C master mode ONLY if there are external slaves (mag/baro) to read.
    // With no slaves present, leaving master mode enabled (and waiting for external
    // sensors) can stall the MPU's accel/gyro sampling -> data reads return 0.
    if (isMagnetometerPresent || isBarometerPresent) {
        mpu6050SetWaitForExternalSensorEnabled(true);
        mpu6050SetI2CMasterModeEnabled(true);
    } else {
        mpu6050SetWaitForExternalSensorEnabled(false);
...
bool sensorsMpu6050Hmc5883lMs5611Test(void)
{
    ...
    // The strict factory self-test is unreliable on clone modules (MPU-6500
    // core, WHO_AM_I 0x70): if it fails, still pass as long as the IMU answers
    // on I2C, so the boot does not stall for minutes on a failing self-test.
    for (int i = 0; i < 300; i++) {
        if (mpu6050SelfTest() == true) {
            isMpu6050TestPassed = true;
            break;
        } else if (mpu6050TestConnection() == true) {
            // clone IMU: self-test trim is not meaningful, connection is enough
            isMpu6050TestPassed = true;
            break;
        } else {
            vTaskDelay(M2T(10));
...
#ifdef GYRO_BIAS_LIGHT_WEIGHT
            if (processGyroBiasNoBuffer(g.x, g.y, g.z, &gyroBias)) {
#else
